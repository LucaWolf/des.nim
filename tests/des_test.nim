...
when isMainModule:
    ...
    var des_test: seq[desVector] = @[
        ...
        ("T10", 0x1818181818181818'u64, bin.fromHex("0707070707070707"),
            fmt_hex, "0808080808080808", "947d60582689287D"),
        
        ("T11", 0x1818181818181818'u64, bin.fromHex("0707070707070707"),
        ...
        # 2DES key suite
        ("T12", 0x9b1c1d0e77021df6'u64, bin.fromHex("80808080010101014040404002020202"),
            fmt_hex, "FEFEFEFE01020408", "cf2c5238010822f1"),
        
        ("T13", 0xe5d21917db50b297'u64, bin.fromHex("80808080010101014040404002020202"),
        ...
        ("T14", 0x5502cdeea963c126'u64, bin.fromHex("b59520682eac25f2ae07819bc0bee7f7"),
        ...
        ("T15", 0xfdf11a883b7b19fe'u64, bin.fromHex("b0437abe0e06206f435b3ac3e9ddac62"),
        ...
        ("T16", 0x435a310855d2fafb'u64, bin.fromHex("91d5886d97efa59b82fd20c3733ca114"),
            fmt_text, "Sto bene, non ti preoccupare!\x00\x00\x00", "617fbb34591c8def389b202cadddfd9e57573471a39e43b515816037a319c9fb"),
        
        #3DES key suite
        ("T17", 0xd961fdb4c5eecbf7'u64, bin.fromHex("617fbb34591c8def389b202cadddfd9e57573471a39e43b5"),
            fmt_hex, "FEDCBA09040302018070605040302010", "640bc66f68ee47e8a73eb8b7f9891244"),
        
        ("T18", 0x9c63a97477f670d0'u64, bin.fromHex("91d5886d97efa59b82fd20c3733ca114728a0a51f0edc81a"),
        ...
        ("T19", 0x7afba9a099c10130'u64, bin.fromHex("df28d4f2aace1b028b1a1e8b0d9bc51bd79aeda555de70ea"),
        ...
        ("T20", 0xc087842e60130df2'u64, bin.fromHex("87926cb6dadb6ce1303cb36148b8257950ddddd4226b2929"),
    ...
    for v in des_test:
        ...
        if v.fmt == fmt_hex:
            desCrypter.encrypt(bin.fromHex(v.clear), output, mode)
        else:
            desCrypter.encrypt(v.clear, output, mode)

        doAssert(output == bin.fromHex(v.cipher), "\n$1: enc [$2] vs cipher [$3] " % [v.desc, output.toHex(false), v.cipher])

        # decrypt suite
        desCrypter.setIV(v.iv)

        desCrypter.decrypt(bin.fromHex(v.cipher), output, mode)

        if v.fmt == fmt_hex:
            doAssert(output == bin.fromHex(v.clear), "\n$1: dec [$2] vs clear [$3] " % [v.desc, output.toHex(false), v.clear])
        else:
